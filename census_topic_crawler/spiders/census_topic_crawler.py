...
class MySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if response.url == 'http://www.census.gov/topics/preparedness.html':
            ...
            request = scrapy.Request('http://www.census.gov/topics/preparedness/about.html',
                callback=self.parse_emerg_prep_about,
                 meta={'item': item},
                 dont_filter=True)
            ...
            parent_topic_ids = response.xpath('//div[contains(@id, "accd_a") and not \
                (@id="NAV_437121255_0_accd_a")]/@id').extract()
            for parent_topic_id in parent_topic_ids:
                item = ParentTopic()
                item['topic_type'] = 'parent'
                item['ID'] = parent_topic_id
                print('parent_topic_id: {}'.format(parent_topic_id))
                parent_topic_name = response.xpath('//div[@id="'+parent_topic_id+'"]\
                    //h4[@class="mobileMenuEntry2"]/text()').extract()[0].strip()
                item['name'] = parent_topic_name
                main_link = response.xpath('//div[@id="'+parent_topic_id[:-8]+'0TabContent"]\
                    //a[contains(text(), "Main")]//@href')[0].extract()
                if parent_topic_id in ['NAV_246571490_0_accd_a', 'NAV_1082186784_0_accd_a']:
                    request = scrapy.Request('http://www.census.gov/econ/progoverview.html',
                        callback=self.parse_parent_overview,
                        meta={'item': item},
                        dont_filter=True)
                elif parent_topic_id == 'NAV_1207455997_0_accd_a':
                    request = scrapy.Request('http://www.census.gov/govs/definitions/',
                        callback=self.parse_parent_definitions,
                        meta={'item': item},
                        dont_filter=True)
                else:
                    request = scrapy.Request(main_link,
                        callback=self.parse_parent_main,
                        meta={'item': item},
                        dont_filter=True)
                request.meta['item'] = item
                items.append(item)
    ...
    def parse_parent_main(self, response):
        ...
        if item['ID'] == 'NAV_820029714_0_accd_a':
            ...
            return scrapy.Request(response.url[:-10]+'reference/definitions/index.html',
                callback=self.parse_parent_definitions,
                meta={'item': item})
        else:
            ...
            main_content = ' '.join(main_content.split())
            item['main_content'] = main_content
            return scrapy.Request(response.url[:-5]+'/about.html',
                callback=self.parse_parent_about,
                meta={'item': item})
    ...
    def parse_parent_about(self, response):
        ...
        about_content = response.xpath('//*[contains(concat( " ", @class, " " ), \
            concat( " ", "textimageText", " " )) and (((count(preceding-sibling::*) + 1) = 1) \
        ...
        if response.url == 'http://www.census.gov/topics/education/about.html':
            subtopics = response.xpath('//*[(@id = "detailContent")]//li//a[(((count(preceding-sibling::*) + 1) = 1)\
                and parent::*)]/text()').extract()
        else:
        ...
        for subtopic in subtopics:
            st = {}
            st['name'] = subtopic
            subtopic_description = response.xpath('//*[(@id = "detailContent")]//\
                li[contains(a/text(), "'+subtopic+'")]/text()').extract()
    ...
    def parse_parent_news(self, response):
        ...
        item = response.meta['item']
        excepts = ['http://www.census.gov/topics/income-poverty/news-updates/news.html',
            'http://www.census.gov/topics/economy/news.html',
            'http://www.census.gov/topics/business/news.html']
        ...
        for article in articles:
            ...
            art['title'] = response.xpath('//div[@id="listArticlesContainer"]//div[@class="textColumn"]\
                //div[@class="title"]//a[@href="'+article+'"]/@title').extract()[0].strip()
            date = response.xpath('//div[@class="article row nested-row no-gutter"]\
                //div[contains(div/a/@href, "'+article+'")]//div[@class="publishdate"]//text()').extract()[0].strip()
            date = datetime.datetime.strptime(date, '%B %d, %Y').strftime('%m/%d/%Y')
            art['date'] = date
            art['description'] = response.xpath('//div[@class="article row nested-row no-gutter"]\
                //div[contains(div/a/@href, "'+article+'")]//div[@class="abstract hidden-xs"]//text()').extract()[0].strip()
    ...
    def parse_parent_survey(self, response):
        ...
        for survey in surveys:
            sur = {}
            sur['link'] = survey
            sur['title'] = response.xpath('//div[@id="listArticlesContainer"]\
            //div[@class="textColumn"]//div[@class="title"]//a[@href="'+survey+'"]/@title').extract()[0].strip()
            sur['description'] = response.xpath('//div[@class="article row nested-row no-gutter"]\
                //div[contains(div/a/@href, "'+survey+'")]//div[@class="abstract hidden-xs"]\
    ...
    def parse_parent_overview(self, response):
        ...
        item = response.meta['item']
        overview_content = response.xpath('//*[@id="middle-column"]//p/text() | \
            //*[@id="middle-column"]//h2/text() | //*[@id="middle-column"]//h4/text() | \
            //*[@id="middle-column"]//h3/text() | //*[@id="middle-column"]//strong/text() | \
            //*[@id="middle-column"]//a/text()').extract()
        overview_content = ''.join(overview_content)
        overview_content = ' '.join(overview_content.split())
    ...
    def parse_parent_definitions(self, response):
        ...
        def_content = ' '.join(def_content.split())
    ...
    def parse_emerg_prep_about(self, response):
        ...
        about_content = ' '.join(about_content.split())
        item['about_content'] = about_content
        return scrapy.Request('http://www.census.gov/topics/preparedness/related-sites.html',
            callback=self.parse_emerg_prep_related_sites,
            meta={'item': item})

    def parse_emerg_prep_related_sites(self, response):
        print('IN PARSE_EMERG_PREP_RELATED_SITES')
        item = response.meta['item']
        related_sites = response.xpath('//*[(@id = "listArticlesContainer")]//*\
            [contains(concat( " ", @class, " " ), concat( " ", "hidden-xs", " " ))]/text() \
            | //*[(@id = "listArticlesContainer")]//a/text()').extract()
        related_sites = ''.join(related_sites)
        related_sites = ' '.join(related_sites.split())
        related_sites = related_sites
        item['related_sites'] = related_sites
        return scrapy.Request('http://www.census.gov/topics/preparedness/surveys-programs.html',
            callback=self.parse_parent_survey,
            meta={'item': item})
